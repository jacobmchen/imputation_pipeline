...
The folder ``exploratory_analysis`` contains code for some exploratory analyses regarding values of biomarkers standardized by urinary creatinine for AKI and no-AKI populations as well as analyses for different severities of AKI.
...
Below are additional analyses we have added on top of the main one above.
...
We would like to expand our definition for AKI beyond just an increase in serum creatinine of 0.3 48 hours post surgery. First, we are going to look at change in creatinine both 48 hours and 72 hours after surgery. If either change in creatinine 48 hours or 72 hours post-surgery satisfy the criteria, then we count it as AKI. Next, we are going to look at the following four definitions of AKI that differentiate between severity:
1. Increase of 0.3 from baseline in serum creatinine.
2. Increase of 50%-99% from baseline in serum creatinine.
3. Increase of 100%-199% from baseline in serum creatinine.
4. Increase of 200%-299% from baseline in serum creatinine.
5. Increase of 300% or more from baseline in serum creatinine.
This exploratory analysis is conducted in ``aki_severity.Rmd``.

Criteria 2-5 are too strict and result in very few patients fulfilling these criteria. 8 patients fulfill criterion 2, 1 patient fulfills criterion 3, and no patients fulfill criteria 4 and 5.

The csv file containing which patients fulfill the criteria above is ``serum_creat_AKI.csv``.
