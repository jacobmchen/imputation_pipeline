...
```{r}
# check whether criterion 1 and 2 capture the same group of patients
AKI_data <- cleaned_creatinine_data %>% filter(AKI_delta == 1)

AKI1_data <- cleaned_creatinine_data %>% filter(AKI_1 == 1)

print(AKI_data$PID)

print(AKI1_data$PID)

# iterate through each PID in the AKI_1 criterion and check if each
# is in the list of PIDs for the AKI_delta criterion
for (id in AKI1_data$PID) {
  if (!(id %in% AKI_data$PID)) {
    print(id)
  }
}
print("finish")
