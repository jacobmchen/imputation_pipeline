...
merged_data <- merged_data %>%
  ...
  mutate(`MCP-1_stand` = `MCP-1` / urcrea) %>%
  mutate(`YKL-40_stand` = `YKL-40` / urcrea)

head(merged_data)
```

```{r}
# this code block is for something else, can skip
...
pivoted_data <- merged_data %>%
  ...
  mutate(collection_time = as.double(sub(".*T", "", sample))) %>%
  select(-sample) %>%
  pivot_wider(names_from = collection_time, values_from = c(urcrea, `KIM-1`, `MCP-1`, NGAL, `YKL-40`, `KIM-1_stand`, `NGAL_stand`, `MCP-1_stand`, `YKL-40_stand`))

head(pivoted_data)
```

```{r}
# compute the difference between the four biomarkers at T1, T2, T3 and T0 to get
...
cleaned_data <- pivoted_data %>%
  ...
  mutate(delta_MCP1_stand = `MCP-1_stand_1` - `MCP-1_stand_0`) %>%
  mutate(delta_MCP1_stand_T2 = `MCP-1_stand_2` - `MCP-1_stand_0`) %>%
  mutate(delta_MCP1_stand_T3 = `MCP-1_stand_3` - `MCP-1_stand_0`) %>%
  mutate(delta_MCP1_stand_T4 = `MCP-1_stand_4` - `MCP-1_stand_0`) %>%
  mutate(delta_YKL40_stand = `YKL-40_stand_1` - `YKL-40_stand_0`) %>%
  mutate(delta_YKL40_stand_T2 = `YKL-40_stand_2` - `YKL-40_stand_0`) %>%
  mutate(delta_YKL40_stand_T3 = `YKL-40_stand_3` - `YKL-40_stand_0`) %>%
  mutate(delta_YKL40_stand_T4 = `YKL-40_stand_4` - `YKL-40_stand_0`) %>%
  select(c(PID, delta_KIM1_stand, delta_NGAL_stand, delta_KIM1_stand_T2,
           ...
           delta_NGAL_stand_T4, delta_MCP1_stand, delta_MCP1_stand_T2,
           delta_MCP1_stand_T3, delta_MCP1_stand_T4,
           delta_YKL40_stand, delta_YKL40_stand_T2, delta_YKL40_stand_T3,
           delta_YKL40_stand_T4))
...
```{r}
write.csv(cleaned_data, "standardized_biomarker_data.csv", row.names=FALSE)
```
